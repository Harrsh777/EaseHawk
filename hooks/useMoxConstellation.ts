...
export function useMoxConstellation(
  ...
  useEffect(() => {
    ...
    nodes.forEach((node) => {
      ...
      pts.push([sx, sy]);
      links += `<line class="mox-link" style="stroke:url(#mlg);stroke-width:1.3" x1="${C}" y1="${C}" x2="${sx}" y2="${sy}"/>`;
    });

    for (let i = 0; i < pts.length; i++) {
      const a = pts[i];
      const b = pts[(i + 1) % pts.length];
      links += `<line class="mox-link" style="stroke:rgba(255,255,255,.07)" x1="${a[0]}" y1="${a[1]}" x2="${b[0]}" y2="${b[1]}"/>`;
    }
